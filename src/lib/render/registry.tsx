...
import { Stars as DreiStars, Text as DreiText, OrbitControls } from "@react-three/drei";
...
interface Animation3D {
  rotate?: Array<number> | null;
}

interface Mesh3DProps {
  position?: Array<number> | null;
  rotation?: Array<number> | null;
...
function toVec3(v: Array<number> | null | undefined): Vec3Tuple | undefined {
  if (!v || v.length < 3) return undefined;
  return v.slice(0, 3) as Vec3Tuple;
}

function toGeoArgs<T extends Array<unknown>>(v: Array<number> | null | undefined, fallback: T): T {
...
function AnimatedGroup({
  ...
  return (
    <group ref={ref} position={toVec3(position)} rotation={toVec3(rotation)} scale={toVec3(scale)}>
      {children}
    </group>
  );
}
  ...
  components: {
          ...
          <TableHeader>
            <TableRow>
              {props.columns.map((col) => {
                const SortIcon =
                  sortKey === col.key ? (sortDir === "asc" ? ArrowUp : ArrowDown) : ArrowUpDown;
                return (
    ...
    RadioGroup: ({ props, bindings }) => {
      ...
      return (
        <div className="flex flex-col gap-2">
          ...
          <RadioGroup value={current} onValueChange={(v: string) => setValue(v)}>
            {(props.options ?? []).map((opt) => (
              <div key={opt.value} className="flex items-center gap-2">
                <RadioGroupItem value={opt.value} id={`rg-${opt.value}`} />
                <Label htmlFor={`rg-${opt.value}`} className="font-normal cursor-pointer">
                  {opt.label}
                </Label>
              </div>
            ))}
    ...
    Scene3D: ({ props, children }) => (
      ...
      >
        ...
        >
          <OrbitControls autoRotate={props.autoRotate ?? false} enablePan enableZoom />
          {children}
        </Canvas>
      </div>
    ),
    ...
    Cylinder: ({ props, emit }) => (
      <MeshPrimitive meshProps={props} onClick={() => emit("press")}>
        <cylinderGeometry
          args={toGeoArgs<[number, number, number, number]>(props.args, [1, 1, 2, 32])}
        />
      </MeshPrimitive>
    ),
    ...
    Torus: ({ props, emit }) => (
      <MeshPrimitive meshProps={props} onClick={() => emit("press")}>
        <torusGeometry
          args={toGeoArgs<[number, number, number, number]>(props.args, [1, 0.4, 16, 100])}
        />
      </MeshPrimitive>
    ),
    ...
    AmbientLight: ({ props }) => (
      <ambientLight color={props.color ?? undefined} intensity={props.intensity ?? 0.5} />
    ),
